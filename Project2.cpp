...

{
	...
	double sumMegaBodies = 0., maxMegaBodies = 0.;
...
#pragma omp parallel for reduction(+:fx,fy,fz) schedule(dynamic)
		...
		sumMegaBodies += megaBodies;
		if (megaBodies > maxMegaBodies)
			maxMegaBodies = megaBodies;

		

	}

	double avgMegaBodies = sumMegaBodies/ (double)NUMTRIES;
	//	prnintf("Number of Node %d", NUMNODES);
	printf("Peak Performance = %8.2lf MegaBodiesSec\n", maxMegaBodies);
	printf("Average Performance = %8.2lf MegaBodies/Sec\n", avgMegaBodies);
